<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="activity[(@type='test-case' or @type='smart-folder' or @type='iteration-container' or @type='entry-container')
        ...
        <li class="{@result} ui-treeList-item {$activityclassname}" id="container{@rid}" data-rid="{@rid}" data-childSuccessCount="{@childsuccesscount}" data-childFailedCount="{@childfailedcount}" data-childBlockedCount="{@childblockedcount}" data-totalSuccessCount="{@totalsuccesscount}" data-totalFailedCount="{@totalfailedcount}" data-totalBlockedCount="{@totalblockedcount}">
    ...
    <xsl:template name="globalCategorySelector">
        <!-- Define variables -->
        <xsl:variable name="successFilter" select="./activity/@filtersuccess"/>
        <xsl:variable name="failedFilter" select="./activity/@filterfailed"/>
        <xsl:variable name="blockedFilter" select="./activity/@filterblocked"/>
        
        <div class="filter">
            <span>Test container filter:</span>
        
            <!-- Success Checkbox -->
            <input type="checkbox" id="checkBoxTestCaseSuccess" name="checkBoxTestCaseSuccess" 
                onClick="OnFilter(this,['li.Success'])">
                <xsl:if test="$successFilter = 'True'">
            ...
            <!-- Failed Checkbox -->
            <input type="checkbox" id="checkBoxTestCaseFailed" name="checkBoxTestCaseFailed" 
                onClick="OnFilter(this,['li.Failed','li.Active'])">
                <xsl:if test="$failedFilter = 'True'">
                    <xsl:attribute name="checked">checked</xsl:attribute>
                </xsl:if>
            ...
            <!-- Blocked Checkbox -->
            <input type="checkbox" id="checkBoxTestCaseIgnored" name="checkBoxTestCaseIgnored" 
                onClick="OnFilter(this,['li.Ignored'])">
                <xsl:if test="$blockedFilter = 'True'">
                    <xsl:attribute name="checked">checked</xsl:attribute>
                </xsl:if>
            </input>
